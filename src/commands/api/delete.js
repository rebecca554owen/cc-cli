...
class DeleteCommand {
  ...
  async deleteSite(allConfigs) {
    ...
    // 选择要删除的站点
    const siteChoices = Object.entries(allConfigs.sites).map(([key, config]) => {
      const icon = getSiteIcon(key, config);
      const tokenCount = Object.keys(config.config?.env?.ANTHROPIC_AUTH_TOKEN || {}).length;
      return {
    ...
    console.log(chalk.gray(`ANTHROPIC_BASE_URL: ${siteConfig.config?.env?.ANTHROPIC_BASE_URL}`));
    console.log(chalk.gray(`Token数量: ${Object.keys(siteConfig.config?.env?.ANTHROPIC_AUTH_TOKEN || {}).length}个`));
  ...
  async deleteToken(allConfigs) {
    ...
    console.log(chalk.gray(`Token值: ${tokens[selectedToken].substring(0, 20)}...`));

    // 检查是否为当前使用的Token
    const currentConfig = await this.configManager.getCurrentConfig();
    const isCurrentToken = currentConfig && 
    ...
    try {
      // 删除Token
      delete allConfigs.sites[selectedSite].config.env.ANTHROPIC_AUTH_TOKEN[selectedToken];
