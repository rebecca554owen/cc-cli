...
class BackupManager {
  ...
  async uploadToWebDAV(backupData, selectedCategories) {
    try {
      ...
      // 4. 上传新备份
      const remotePath = await this.webdavClient.uploadBackup(fileName, backupData);
  ...
  generateBackupFileName(selectedCategories) {
    ...
    const categoryPrefix = selectedCategories.length === 1
  ...
  async cleanupOldBackups() {
    try {
      ...
      if (backups.length >= maxBackups) {
        ...
        console.log(chalk.blue(`🧹 清理旧备份文件 (保留最新${maxBackups}个)...`));

        for (const backup of backupsToDelete) {
          await this.webdavClient.deleteBackup(backup.path);
        }

        console.log(chalk.green(`✅ 已清理 ${backupsToDelete.length} 个旧备份文件`));
      }
    } catch (error) {
